...
schedulertest(void)
{
    ...
    for(n=0; n<3; n++){
        ...
        if(pid == 0){
            ...
            printf(1, "Process #%d with %d tickets, %d stride and %d current stride finished and was executed %d times.\n", getpid(), gettickets(), getstride(), getcurrentstride(), getsortednumber());
            exit();
        }
    }
